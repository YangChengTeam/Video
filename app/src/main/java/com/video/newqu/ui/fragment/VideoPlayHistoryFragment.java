...
//        page=0;
        ...
        if(1!= SharedPreferencesUtil.getInstance().getInt(Constant.TIPS_USER_HISTORY_CODE)){
            new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
                    if(null!=VideoPlayHistoryFragment.this.getActivity()&&!VideoPlayHistoryFragment.this.getActivity().isFinishing()){
                        //删除视频提示
                        new android.support.v7.app.AlertDialog.Builder(VideoPlayHistoryFragment.this.getActivity())
