...
public class MediaMusicRecommendFragment extends BaseLightWeightFragment<FragmentMediaMusicHomeBinding,MediaMusicRecommendPresenter> implements MediaMusicRecommendContract.View,OnMediaMusicClickListener{
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        mPresenter = new MediaMusicRecommendPresenter(getActivity());
    ...
    protected void onVisible() {
        super.onVisible();
        if(isRefresh&&null!=bindingView&&null!=mMusicAdapter&&null!=mPresenter&&!mPresenter.isHomeLoading()){
    ...
    private void initAdapter() {
        List<MediaMusicCategoryList.DataBean> cacheList= (List<MediaMusicCategoryList.DataBean>)  ApplicationManager.getInstance().getCacheExample().getAsObject(Constant.CACHE_MEDIA_RECORED_RECOMMEND_MUSIC);
        ...
        mMusicAdapter.setOnLoadMoreListener(new BaseQuickAdapter.RequestLoadMoreListener() {
            ...
            public void onLoadMoreRequested() {
                if(null!=mMusicAdapter){
                    List<MediaMusicCategoryList.DataBean> data = mMusicAdapter.getData();
                    if(null!=data&&data.size()>9&&null!=mPresenter&&!mPresenter.isHomeLoading()){
    ...
    private void loadMusicData() {
        if(null!= mPresenter &&!mPresenter.isHomeLoading()){
            mPage++;
            mPresenter.getRecommendMusicList(null,mPage,pageSize);
        }
    }
    ...
    public void showRecommendMusicList(List<MediaMusicCategoryList.DataBean> data) {
        ...
        if(null!=mEmptyViewbindView) mEmptyViewbindView.emptyView.showEmptyView("暂无音乐数据",R.drawable.iv_work_video_empty,false);
        if(null!=mMusicAdapter){
            ...
            if(1==mPage){
                ...
                ApplicationManager.getInstance().getCacheExample().put(Constant.CACHE_MEDIA_RECORED_RECOMMEND_MUSIC, (Serializable) data, Constant.CACHE_TIME);
                //添加数据
            }else{
